...
　　your url?key=value
    ...
    def Get_Url(self):
        ...
        if config.has_section("start_urls"):
            try:
                ...
                self.file_log.debug("读取到{}的配置为{}".format("start_urls", self.start_urls))
    ...
    def Lower_Page(self):
        ...
        try:
            for url in self.start_urls:
                ...
                self.requests_url = self.Get_Request_Url(temp_,'a',"href","title")
    ...
    def Wildcard_Character(self):
        ...
        try:
            for key in self.requests_url:
                ...
                #     f.write(temp)
                temp = etree.HTML(temp)  # 转变为一个etree对象
                temp_ = temp.xpath("/html/body/div[3]/div/div/div[3]/div/div[2]")
                # print(temp_)
                ...
                for u in page_url:
                    ...
                    temp = etree.HTML(temp)  # 转变为一个etree对象
                    try:
                        temp_ = temp.xpath(self.Config_Dict["wildcard_url_xpath"])
                        self.download_urls = self.Get_Request_Url(temp_,"img","src","alt")
                        try:
                            self.Dict_Combination(key)
                        except KeyError:
                            self.file_log.warning("没有这个键值对")
                    except KeyError:
                        self.file_log.critical("没有找到{}".format(self.Config_Dict["wildcard_url_xpath"]))
                # finally:
                #     print("程序没运行")
        except selenium.common.exceptions.WebDriverException:
            self.file_log.critical("网络异常")
    ...
    def Save(self, stater:str,subtitle: str,image_title:str,picture_data:bytes ):
        print(sys._getframe().f_code.co_name + "开始运行")
        download_main_path = os.path.join("./", self.main_title)  # 主目录的标题  ./美女黑丝
        if os.path.isdir(download_main_path):
            # print("目录:{}存在".format(download_main_path))
            if stater == "image":
                download_title = os.path.join(download_main_path,"Image")  # 副目录的标题     ./美女黑丝/图片/
                if os.path.isdir(download_title):
                    ...
                    if os.path.isdir(download_title1):
                        ...
                        try:
                            ...
                            page_path = os.path.join(download_title1, page_path)
                            # print(page_path)
                            with open(page_path, "wb") as page_name:
                                page_name.write(picture_data)
                                self.file_log.debug("{}下载完成".format(page_path))
                        except FileNotFoundError:
                            self.file_log.warning("{}.jpg文件创建失败".format(image_title))
                        except TypeError:
                            self.file_log.warning("{}.jpg文件写入失败".format(image_title))
                    else:
